...
mitjançant una plana web hostatjada en un servidor web tipus *localhost*.

## Presentació del llenguatge Funx
...
Això implica que si trobem més d'una expressió, tindrem més d'un punt de 
...
condicions com als procediments i als `while`s).

## Iteració amb `while`

La instrucció iterativa amb `while` té la semàntica habitual. Exemple: 
`while a > 0 { a <- a / 2 }`.
...
error. Les funcions es poden cridar recursivament. La sintàxi és com a Haskell: 
sense parèntesis ni comes.
Exemple: `Suma x + y 2`.
...
No heu de fer una anàlisi semàntica per errors de tipus.
...
Utilitzeu `ANTLR` i `flask` per escriure la gramàtica i l'intèrpret. Podeu 
utilitzar lliurament qualsevol llibreria estàndard de Python.
...
- Un fitxer `*.html` en una carpeta `templates` amb la plantilla `jinja` de la 
vostra pràctica.
...
programes en JSBach han de seguir l'estil exposat en aquest document.
